...
In the spirit (👻) of the season and keeping things spooky, we've built a digital, online version of the original spiritualists' tool to help you communicate with the dead ⚰️ (or maybe just the ghosts in the machine).
...
- To install the [azure-functions-core-tools](https://github.com/Azure/azure-functions-core-tools)
...
The web app uses [Ably](https://www.ably.io/) for [pub/sub messaging](https://www.ably.io/documentation/core-features/pubsub) between the users. Ably is an enterprise-ready pub/sub messaging platform that makes it easy to design, ship, and scale critical realtime functionality directly to your end-users. In this case, We're using it to send messages between users when they move their mouse.
...
We'll use Azure functions to host our API, so you'll need the Azure functions core tools. Run:
...
We're going to build this application as a client side web app with HTML, CSS and Typescript. We'll use Ably realtime messaging to communicate between the users. We'll use Snowpack as our dev server and build tool, it will let us write TypeScript that is transparently compiled to JS for our browsers.

We need our web app to do two things:

1. Let the users create and name a board to join with their friends
2. Render our spirit board and planchette

This will need two distinct interfaces: a form to name and join a board and the board itself.

Our entire UI will fit inside our index.html file, so lets start by looking at the form in [index.html](/app/index.html).
...
This is a regular HTML form - clicking the button will redirect the user right back to `index.html` with the `boardName` field value added as a query string parameter. We can then process this in our `index.ts` file.
...
Displaying the board itself is relatively simple, it is a background image on `.board-holder`. Once a session has been joined a we remove the `hidden` class on the `#activeBoard` and add it to the `#joinForm`.

# index.ts

[index.ts](/app/index.ts) is our entry point and contains all the logic to connect to a board and display the right things, it looks like this:
...
First we import all the libraries we need - we've split out a lot of the logic for different parts of the app into their own files - the SpiritBoard logic, JoinBox logic and Networking.
...
Then we create instances of the three components. Notice that we pass the id of the `join` and `activeBoard` elements to the constructors of their respective objects. These objects will perform any DOM manipulation that we need to do.
...
Next, we wire up the callback for to respond to messages being received from other clients. We call `board.movePlanchette()`, with the contents of our message. Notice that each of these messages has a different name - the `Networking` class is handling our Ably subscriptions for us here.
...
Next we wire up some of the effects - when the planchette lands on a letter in the browser, we want to 'reveal' it by playing a spooky animation that we'll implement in the SpiritBoard class and animate in the css.
...
And finally, we're displaying the spirit board, and playing some spooky music, along with registering service worker which will display a page if the app is offline.
...
This index file explains everything that the game does - but not how it does it, so lets dig deeper into some of our classes.

# JoinBox.ts

[JoinBox.js](/app/joinbox.ts) captures our UI interactions with the Join form and it looks like this:
...
Most of the work is done in the constructor - it finds the element in the page (or uses one provided by our tests), and finds the input box that users fill out their desired board name in.

There are a couple of functions that we call from our index - `hide`, `show` and `setBoardName`.

All three of them are pretty self explanatory, and do exactly what they say - `hide` and `show` use CSS classes, and `setBoardName` fills the textbox with whatever parameter is passed to the function.

If you're new to `TypeScript`, you might be wondering what the `:HTMLElement` and `: HTMLInputElement` syntax after our variable declarations is - those are `TypeScript` type annotations - and by using them, you get better auto-complete support and compile time checking when our application gets built.

# SpiritBoard.ts

In [SpiritbBoard.ts](/app/SpiritBoard.ts), the SpiritBoard class contains all of the interactions we can have with our board, each as an exposed `public function`.
...
We're going to walk through the code and unpick what the SpiritBoard does.
...
We also declare a const called `Gyroscope` to represent the browser's `Gyroscope` APIs. (These are new and not quite finalised so don't exist in the currently published Type definitions.)

We declare a `SpiritBoard` class:
...
We create some public variables - one for the `planchette`, and another called `blockMovement`. Next, we declare some private variables to keep track of the board state. Finally, we define the constructor:
...
Just like the `JoinBox`, displaying the spirit board is achieved by removing a `hidden` CSS class. There is a little bit more to rendering the board, because we also have to trigger the drawing of the `Planchette`.
...
This `render()` function calls `render` on the `Planchette`, invokes `raiseEventsForNewlyRevealedItems` (which we'll discuss in a later section) and then using the `requestAnimationFrame()` API, queues a callback to itself. [requestAnimationFrame()](https://developer.mozilla.org/en-US/docs/Web/API/window/requestAnimationFrame) is a browser API for animating page elements - and your browser will call this function many times a second, to match the frame rate of your browser, a framerate that normally matches the refresh rate of your computer's display.
...
The [SpiritBoard](/app/SpiritBoard.ts) handles all of the user input. First, the constructor wires up mouse move events and calls our enableGyroscope function:
...
`monitorMouse` then uses the `Networking` instance to `sendBufferedMessage()` with the movement information from the mouse move event:
...
and enableGyroscope, if a gyroscope is available, adds an event listener to `reading`, at a frequency of 60 times per second:
...
Whenever we send a message using `sendBufferedMessage` it actually puts the message into a variable called `_outboundBuffer`.
If, during the sending of a message, our buffer reaches our `_bufferCap` (set to 150 messages), it'll flush the buffer. Flushing the buffer simply takes the collection of messages, and sends them all at once via Ably in a message with the name `bulk`.

Elsewhere in this class, if a `bulk` message is received, each of these array elements is looped over, and our message handlers called for each individual item:
...
which is hooked up to our Ably messages in `index.ts`, as you can see above, it's not where the logic to actually *move* our planchette resides.

The movement `Deltas` that move the planchette look like this:

```ts
export type Delta = { deltaX: number; deltaY: number; }
```

They are made up of two numbers - a delta X and a delta and are passed onto the `planchette.move()` function.
...
`.move()` isn't particularly interesting - it modifies the current x,y location of the top left of the planchette (stored as a `Coordinate`):

```ts
export type Coordinate = { x: number; y: number; }
```

and then calls a function called `snapToBoundaries`. This function is responsible for making sure that once we've moved the planchette, it hasn't gone flying way out of the boundaries of the board graphic and into the ether. While spooky, this wouldn't really help anyone to use the board! 👻

Snap to boundaries is a little tedious to read line by line - but what it does, is allow the planchette to only cross the boundaries of the board by half of its own width or height the board - we do this to make sure that the planchette can still discover any letters towards the bottom, or far left and right of the board:
...
First we set up some boundaries, and once we've modified the location of the planchette, if it has moved outside of those boundaries, we set its location to the corresponding `snapPoint` - the furthest location the `planchette` should be allowed to go outside of the border.
...
While it might seem like we could detect the revealed letters/numbers once we've moved the planchette (and that would be one way to implement this), our detection is actually separate from the planchette movement. The letter detection is actually driven by the `SpiritBoard.ts` render function:
...
Here, we stop the planchette from responding to user input for three seconds, we focus on the `focalPoint` of our revealed item by moving center of the planchette 'viewfinder' to the center of the detected character, and if there's a callback (like there is in `index.ts`) we'll call it.
...
In this function, we call `detectElementsAt()`, passing in a property called `viewFinderTarget` - (this is the location of the center of the planchette's viewfinder). If something is returned, we read two data attributes from the element - the `item` - literally the letter or phrase the planchette is hovering over, and the `focus` - the visual middle point of that character so that we can move the planchette to pleasingly frame it.

We're lucky that most of the heavy lifting here is actually done by the browser:
...
We start off with a little bit of maths to make sure we have the correct coordinates, then we call the `DOM` API `elementsFromPoint`.

When we call this on an x,y coordinate, it'll return a list of elements under that literal point in the browser. We then filter out any elements that don't have a class of `holder`, and to understand why, we'll need to take a look at our markup again.
...
By outlining the characters with these hit detection polygons, we can make sure that what the users see through the `planchette` is exactly the letter we're detecting and that it feels responsive. (If this feels familiar to you, it's because it's very similar to the way that hit detection works in HTML image maps. *Editor's note: remember the bad old days of giant images and dozens of image maps?!*

* The `data-item` attribute tells us what character is contained.
* The `data-focus` attribute is the coordinates of the center of the polygon (eyeballed in Adobe illustrator).
...
This is how the notifications are connected. The `onReveal()` function looks like this: 
...
It saves a callback, which is triggred by `raiseEventsForNewlyRevealedItems` when a new item is detected.
...
It sets the `innerHTML` of the `animation` div to the text of the detected character, and adds the CSS class `animate`, along with creating a timeout to remove the class in 3 seconds. We then use CSS animations to do all the hard work and give us a seasonally spoopy effect:
...
Over a period of 3 seconds, with a one second delay, we take the contents of the div and enlarge it, skew it back and forth, add a blur and reduce it's opacity until it disappears like smoke from an extinguished candle.
...
One of the wrinkles in using the approach we have, where all of the clients "play" the same messages, and move their planchettes by the same amount, is that one pixel's worth of movement on one screen will be different to one pixel's movement on another. As a result, when we're processing our mouse and gyro movement deltas, we have to make sure they mean the same thing on each computer.

To achieve this, the board operates at an **internal resolution** of 1240x768 - that means that whenever we want to move something "one pixel", it's always calculated as "one pixel at 1240x768".

The `planchette` keeps track of its `location` as (x,y) coordinates, but most of our calculations - like when we work out where the centre of the `planchette` viewfinder is, or when we work out how far over the edge of the board it can travel, are calculated based on the property `scaledLocation`.
...
`scaledLocation` takes the current `this.location` property, and adjusts it for the user's screen resolution - working out what one percent of their horizontal and vertical resolution would be, and adjusting the numbers appropriately. By doing this, we make sure that when we run the collision detection code, we're going to detect the same character on two computers running at different resolutions, where the `planchette` has different absolute (x,y) coordinates on the screen.

It's all a bit of a headache, and it's the complicated part of synchronizing two or more clients with different output resolutions using purely client side calculations. It's a little bit cumbersome, but the alternative approach would be to have only one client detect letters, and raise messages to keep everyone in sync.

For the scope of a small game like this, just playing the events on each client seemed like the simplest path.

# Making the layout responsive

As with any responsive site, we don't know what size of device or screen our users will be viewing the app on. We want to make sure that whatever size the board is, all of the users see the entire board (without a scroll) and see the plancette move to the same letters.

The board itself has a set aspect ratio, which needs to scale with the browser. Which means that we need to take both the width and height of the browser into account.

First we set a maximum width and height for the board so that it won't get too large bigger screens. We don't want to give people eyestrain when looking at the board! I chose 1240 and 768 to give a slightly widescreen graphic. Honestly, mostly these numbers were accidental as I was designing the board.

The board graphic in the browser then has to scale as the window width gets narrower than 1240px and as the height gets shorter than 768px.

![movement gif](https://media.giphy.com/media/lRVZHAsnT07uTnzbj5/giphy.gif "movement gif")

To do this we:

- Make a container element for the board (`.board-holder`).

- Set a `max-width` of 1240px and a `max-height` of 100vh on the container so that it won't get too large or overflow the window height.

- Create a media query which triggers when the height of the browser window is over 768px which decreases the `max-height` to 768px.

- Set the background image container to be a picture of the board

- Use `background-size:contain` to make sure the background image always fits inside the constraints of either width or height;
